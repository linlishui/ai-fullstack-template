...
- OpenSpec 完成后可按 `docs/concurrent-generation.md` 并发推进独立分片，但必须先写 `doc/parallel-execution-plan.md`，明确文件所有权和集成顺序
...
```text
请使用 template-project-driver skill 执行当前模板流程：读取 requirements/requirement.md，先生成 OpenSpec，再按 docs/concurrent-generation.md 规划并发分片，把完整项目输出到 generated/<project-slug>/，最后执行模板级审计与项目级验证。
```
...
| `docs/concurrent-generation.md` | OpenSpec 后的并发生成、文件所有权和集成关口 |
| `docs/production-grade-rubric.md` | 生产级评分硬门禁 |
| `docs/fullstack-review-scoring.md` | 120 分 fullstack reviewer 评分口径与一票否决项 |
| `scripts/capture_screenshots.sh` | Playwright 自动截图工具（从路由提取页面并截图到 doc/screenshots/） |
